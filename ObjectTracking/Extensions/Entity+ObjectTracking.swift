...
extension Entity {
    ...
    func applyPortalRecursively(with world: Entity) {
        self.components.set(PortalComponent(target: world))
        for child in children {
            child.applyPortalRecursively(with: world)
        }
    }
...
extension simd_float4x4 {
    var position: SIMD3<Float> { return SIMD3<Float>(self.columns.3.x, self.columns.3.y, self.columns.3.z) }
}

extension simd_float4 {
    var xyz: SIMD3<Float> { return SIMD3<Float>(self.x, self.y, self.z) }
}
